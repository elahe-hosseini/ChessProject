...
//in tabe bool return kone va jahayi ke getpiecetype hast ye is empty ham ezafe konim
    ...
    private void processCommand(String paramString) {
        if(paramString.compareTo("@BLACK") == 0) {
            this.myColor = 13;
            this.Player.setTitle("Chess Client - BLACK");
            resetBoard();
        }
        else if(paramString.compareTo("@WHITE") == 0) {
            this.myColor = 14;
            this.Player.setTitle("Chess Client - WHITE");
            resetBoard();
            this.myTurn = true;
        }
...
void checkMate(){boolean n=false,m=false;
    for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
            if(chessBoard[i][j]==0){n=true;}if(chessBoard[i][j]==6){m=true;}
        }
    }
    if(n==false||m==false){System.exit(0);}
    }
